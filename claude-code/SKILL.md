...
1. **Bootstrap & maintain** a criteria profile at `~/.claude/jd-triage/jd_criteria.md`.
2. **Evaluate** a JD → two-dimension verdict + one action.
3. **Log** every evaluation to `~/.claude/jd-triage/jd_history.md`.
...
Append to `~/.claude/jd-triage/jd_history.md` — format and ID scheme in `references/history.md`. Create the file if missing.
